...
Este documento describe el esquema de base de datos diseñado para TrainHub basado en `dbdiagram.sql`.
...
└────────┬────────┘
    ...
    │ r1_time       │          │ expiration_date │
    │ ... r8_time   │          │ revoked         │
    │ w1_time       │          │ creation_date   │
    │ ... w8_time   │          └─────────────────┘
    │ total_time    │
    │ creation_date │
    └────┬──────────┘
    ...
    │    Likes      │    │  friendships   │
    ├───────────────┤    ├────────────────┤
    │ user_id (FK)  │    │ user_a_id (FK) │
    │ post_id (FK)  │    │ user_b_id (FK) │
    │ like_date     │    │ status         │
    │ PK(user,post) │    │ relationship_  │
    └───────────────┘    │   start_date   │
                         │ PK(user_a,     │
                         │  user_b)       │
                         └────────────────┘
